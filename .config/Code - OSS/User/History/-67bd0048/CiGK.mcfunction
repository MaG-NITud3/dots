...
execute as @e[type=piglin,predicate=test:piglin_offhand_gold] if score @s Barter matches 1 run scoreboard players add @p Times 1
execute as @e[type=piglin,predicate=test:piglin_offhand_gold] if score @s Barter matches 121 run scoreboard players add @p Times 1